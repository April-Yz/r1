...
python3 bag2h5x.py /home/pine/yzj/pour/r1_data_20260127_154637.bag --output /home/pine/yzj/h5out/r1_data_20260127_154637.h5
"""
...
def bag_to_h5(bag_path, output_path=None):
    ...
    for key in data:
        ...
        # 保存gripper control action到aligned_data
    ...
    if len(data['gripper_left_timestamps']) > 0:
        all_timestamps.extend(data['gripper_left_timestamps'])
    if len(data['gripper_right_timestamps']) > 0:
        all_timestamps.extend(data['gripper_right_timestamps'])
    if len(data['action_arm_left_timestamps']) > 0:
        all_timestamps.extend(data['action_arm_left_timestamps'])
    if len(data['action_arm_right_timestamps']) > 0:
        all_timestamps.extend(data['action_arm_right_timestamps'])
    ...
    if len(data['arm_left_pos']) > 0:
        ...
        aligned_data['obs_arm_left_pos'] = np.array([f(uniform_timeline) for f in interp_funcs]).T
    ...
    if len(data['arm_right_pos']) > 0:
        ...
        aligned_data['obs_arm_right_pos'] = np.array([f(uniform_timeline) for f in interp_funcs]).T
    
    if len(data['gripper_left_pos']) > 0:
        print("  Interpolating left gripper joint positions (obs)...")
        interp_funcs = [interp1d(data['gripper_left_timestamps'], data['gripper_left_pos'][:, i], 
                                kind='linear', bounds_error=False, fill_value='extrapolate')
                       for i in range(data['gripper_left_pos'].shape[1])]
        aligned_data['obs_gripper_left_pos'] = np.array([f(uniform_timeline) for f in interp_funcs]).T
    
    if len(data['gripper_right_pos']) > 0:
        print("  Interpolating right gripper joint positions (obs)...")
        interp_funcs = [interp1d(data['gripper_right_timestamps'], data['gripper_right_pos'][:, i], 
                                kind='linear', bounds_error=False, fill_value='extrapolate')
                       for i in range(data['gripper_right_pos'].shape[1])]
        aligned_data['obs_gripper_right_pos'] = np.array([f(uniform_timeline) for f in interp_funcs]).T
    ...
    if len(data['action_arm_left_pos']) > 0:
        ...
        interp_funcs = [interp1d(data['action_arm_left_timestamps'], data['action_arm_left_pos'][:, i], 
                                kind='linear', bounds_error=False, fill_value='extrapolate')
                       for i in range(data['action_arm_left_pos'].shape[1])]
        aligned_data['action_arm_left_pos'] = np.array([f(uniform_timeline) for f in interp_funcs]).T
    
    if len(data['action_arm_right_pos']) > 0:
        print("  Interpolating right arm joint positions (action)...")
        interp_funcs = [interp1d(data['action_arm_right_timestamps'], data['action_arm_right_pos'][:, i], 
                                kind='linear', bounds_error=False, fill_value='extrapolate')
                       for i in range(data['action_arm_right_pos'].shape[1])]
        aligned_data['action_arm_right_pos'] = np.array([f(uniform_timeline) for f in interp_funcs]).T
    
    # Compute gripper action: action[t] = mapped(obs[t+1])
    # Mapping rule: if value > 50, keep unchanged; if value <= 50, multiply by 0.9
    ...
    with h5py.File(output_path, 'w') as f:
        ...
        def save_with_progress(group, name, data, compression=None, compression_opts=None):
            ...
            # Create dataset
            if compression:
                dataset = group.create_dataset(name, shape=data.shape, dtype=data.dtype,
        ...
        # Save observation images with compression
        print("Saving obs/camera_head data...")
        if len(aligned_data['obs_camera_head_rgb']) > 0:
            save_with_progress(obs_group, 'camera_head/rgb', aligned_data['obs_camera_head_rgb'],
                             compression='gzip', compression_opts=4)
        if len(aligned_data['obs_camera_head_depth']) > 0:
            save_with_progress(obs_group, 'camera_head/depth', aligned_data['obs_camera_head_depth'],
                             compression='gzip', compression_opts=4)
        
        print("Saving obs/camera_left data...")
        if len(aligned_data['obs_camera_left_rgb']) > 0:
            save_with_progress(obs_group, 'camera_left/rgb', aligned_data['obs_camera_left_rgb'],
                             compression='gzip', compression_opts=4)
        if len(aligned_data['obs_camera_left_depth']) > 0:
            save_with_progress(obs_group, 'camera_left/depth', aligned_data['obs_camera_left_depth'],
                             compression='gzip', compression_opts=4)
        
        print("Saving obs/camera_right data...")
        if len(aligned_data['obs_camera_right_rgb']) > 0:
            save_with_progress(obs_group, 'camera_right/rgb', aligned_data['obs_camera_right_rgb'],
                             compression='gzip', compression_opts=4)
        if len(aligned_data['obs_camera_right_depth']) > 0:
            save_with_progress(obs_group, 'camera_right/depth', aligned_data['obs_camera_right_depth'],
                             compression='gzip', compression_opts=4)
        
        # Save observation joint positions (small, no progress needed)

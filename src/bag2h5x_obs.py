...
def compute_fk(joint_pos, side):
    ...
    joint_positions_full = np.concatenate([
        ...
        joint_pos[:, :6] if joint_pos.shape[1] > 6 else joint_pos
    ], axis=1)
    try:
        ...
        chain = kp.build_serial_chain_from_urdf(
            urdf_data,
            CHAIN_CONFIG[side]['end_link'],
            CHAIN_CONFIG[side]['root_link']
        )
    except Exception as e:
        print(f"FK chain build error: {e}")
        return None, None, None
    ...
    return eef_pos, eef_quat, eef_euler
#!/usr/bin/env python3
"""
...
python3 bag2h5x_obs.py /media/pine/新加卷/R1/pour --batch --workers 8 --output /home/pine/yzj/h5out_small
...
def bag_to_h5(bag_path, output_path, num_workers=4):
    ...
    with h5py.File(output_path, 'w') as f:
        ...
        for side in ['left', 'right']:
            arm_grp = obs_group.require_group(f'arm_{side}')
            joint_pos = aligned_data.get(f'obs_arm_{side}_pos')
            save_ds(arm_grp, 'joint_pos', joint_pos)
            # FK 计算并保存
            eef_pos, eef_quat, eef_euler = compute_fk(joint_pos, side)
            save_ds(arm_grp, 'eef_pos', eef_pos)
            save_ds(arm_grp, 'eef_quat', eef_quat)
            save_ds(arm_grp, 'eef_euler', eef_euler)
            gripper_grp = obs_group.require_group(f'gripper_{side}')
            save_ds(gripper_grp, 'joint_pos', aligned_data.get(f'obs_gripper_{side}_pos'))

        for cam in ['head', 'left', 'right']:
            cam_grp = obs_group.require_group(f'camera_{cam}')
            save_ds(cam_grp, 'rgb', aligned_data.get(f'obs_camera_{cam}_rgb'), True)
            save_ds(cam_grp, 'depth', aligned_data.get(f'obs_camera_{cam}_depth'), True)

        # action group
        for side in ['left', 'right']:
            arm_grp = action_group.require_group(f'arm_{side}')
            joint_pos = aligned_data.get(f'action_arm_{side}_pos')
            save_ds(arm_grp, 'joint_pos', joint_pos)
            # FK 计算并保存
            eef_pos, eef_quat, eef_euler = compute_fk(joint_pos, side)
            save_ds(arm_grp, 'eef_pos', eef_pos)
            save_ds(arm_grp, 'eef_quat', eef_quat)
            save_ds(arm_grp, 'eef_euler', eef_euler)
            gripper_grp = action_group.require_group(f'gripper_{side}')
            save_ds(gripper_grp, 'joint_pos', aligned_data.get(f'action_gripper_{side}_pos'))
            if f'action_gripper_{side}_cmd' in aligned_data:
                save_ds(gripper_grp, 'commanded_pos', aligned_data[f'action_gripper_{side}_cmd'])
                save_ds(gripper_grp, 'commanded_pos_timestamps', aligned_data[f'action_gripper_{side}_cmd_timestamps'])
